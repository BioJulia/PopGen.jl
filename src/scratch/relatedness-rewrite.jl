function _relatedness_noboot_fix2(data::PopData, sample_names::Vector{String}; method::F, inbreeding::Bool) where F
    ...
    @inbounds Base.Threads.@threads for i in 1:length(sample_pairs)
        ...
        @inbounds [relate_vecs[j][i] = @inbounds mth(geno1[keep_idx], geno2[keep_idx], loci_names[keep_idx], allelefrequencies, loc_n = n_per_loci[keep_idx], n_samples = n_samples, inbreeding = inbreeding) for (j,mth) in enumerate(method)]
        next!(p)
    end
...
function _relatedness_boot_all(data::PopData, sample_names::Vector{String}; method::F, iterations::Int = 100, interval::Tuple{Float64, Float64} = (0.025, 0.975), inbreeding::Bool) where F
    ...
    @inbounds Base.Threads.@threads for i in 1:length(sample_pairs)
        ...
        @inbounds for (j, mthd) in enumerate(method)
            ...
            boot_out = _bootstrapgenos_all(geno1, geno2, loci_names, n_per_loci, allelefrequencies, method = mthd, iterations = iterations, inbreeding = inbreeding)
            @inbounds boot_means[j][i], boot_medians[j][i], boot_ses[j][i], boot_CI[j][i] = _bootstrapsummary(boot_out, interval)
        end
        next!(p)
    end
    ...
    @inbounds for (sample_n, ind1) in enumerate(sample_names[1:end-1])
        ...
        @inbounds @sync Base.Threads.@spawn for ind2 in sample_names[sample_n+1:end]
            ...
            @inbounds for (i, mthd) in enumerate(method)
                @inbounds relate_vecs[i][idx] = mthd(gen1, gen2, loc, allelefrequencies, loc_n = n_per_loci, n_samples = n_samples, inbreeding = inbreeding)
                boot_out = _bootstrapgenos_all(geno1, geno2, loci_names, n_per_loci, allelefrequencies, method = mthd, iterations = iterations, inbreeding = inbreeding)
