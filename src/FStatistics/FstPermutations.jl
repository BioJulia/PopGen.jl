function _permuted_WeirCockerham(data::PopData, iterations::Int64)
    ...
    for i in 2:npops
        for j in 1:(i-1)
            ...
            @sync for iter in 1:iterations-1
                Base.Threads.@spawn begin
                    perm_p1, perm_p2 = wc_permutation(pop1,pop2)
                    perm_vector[iter] = weircockerham_fst(perm_p1, perm_p1)
                end
            end
            results[i,j] = wc_val
            results[j,i] = (sum(wc_val .<= perm_vector) + 1) / iterations
        end
    end
...
function wc_permutation(population_1::T, population_2::T) where T<:AbstractMatrix
    ...
    perm2 =(1:n_samples)[Not(perm1)]
    # index the merged matrix with the permuted indices
    new_pop_1 = merged[perm1,:]
    new_pop_2 = merged[perm2,:]
